...
class DatabaseConfig(BaseConfig):
    ...
    password: str = Field(
        default='', description='数据库密码', validation_alias='DB_PASSWORD'
    )

    database: str = Field(
        default='app', description='数据库名称', validation_alias='DB_NAME'
    )

    # 连接池配置
    pool_size: int = Field(
        default=20, description='连接池大小', validation_alias='DB_POOL_SIZE'
    )

    max_overflow: int = Field(
        default=10, description='最大溢出连接数', validation_alias='DB_MAX_OVERFLOW'
    )

    pool_timeout: int = Field(
        default=30, description='连接池超时(秒)', validation_alias='DB_POOL_TIMEOUT'
    )

    pool_recycle: int = Field(
        default=3600, description='连接回收时间(秒)', validation_alias='DB_POOL_RECYCLE'
    )
